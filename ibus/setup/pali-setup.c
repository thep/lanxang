...
static void
ibus_lanxang_pali_disconnected_cb (IBusBus* bus, gpointer user_data)
{
  exit(2);
}
...
main (int argc, char **argv)
{
  ...
  context = g_option_context_new ("- ibus lanxang Lao Pali setup component");
  ...
  gtk_window_set_default_icon_from_file (PKGDATADIR"/icons/ibus-lanxang-pali.svg",
                                         NULL);
  main_dlg = ibus_lanxang_pali_setup_dialog_new ();
  g_signal_connect (main_dlg, "destroy", gtk_main_quit, NULL);

  ibus_init ();
  bus = ibus_bus_new ();
  g_signal_connect (bus, "disconnected",
                    G_CALLBACK (ibus_lanxang_pali_disconnected_cb), NULL);
  ibus_lanxang_init_config ();

  ibus_lanxang_read_pali_config (&opt);
  ibus_lanxang_pali_setup_set_values (GTK_DIALOG (main_dlg), &opt);

  ret = gtk_dialog_run (GTK_DIALOG (main_dlg));
  if (GTK_RESPONSE_OK == ret)
    {
      ibus_lanxang_pali_setup_get_values (GTK_DIALOG (main_dlg), &opt);
      ibus_lanxang_write_pali_config (&opt);
    }
