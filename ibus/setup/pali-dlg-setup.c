...
GtkWidget *
ibus_lanxang_pali_setup_dialog_new ()
{
  GtkWidget *main_dlg;
  GtkWidget *main_vbox;
  GtkWidget *widget;
  ...
  /* Create main window */
  main_dlg = gtk_dialog_new_with_buttons (_("IBus LanXang Lao Pali Preferences"),
...
void
ibus_lanxang_pali_setup_set_values (const GtkDialog *dlg,
                                    const IBusLanXangSetupOptions *opt)
{
  GtkWidget *widget;

  /* Set input sequence check value */
  switch (opt->pali_isc_mode)
    {
...
void
ibus_lanxang_pali_setup_get_values (const GtkDialog *dlg,
                                    IBusLanXangSetupOptions *opt)
{
  ...
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
    {
      opt->pali_isc_mode = ISC_PASSTHROUGH;
      goto pali_isc_mode_done;
    }
  widget = g_object_get_data (G_OBJECT (dlg), "strict_check_radio");
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
    {
      opt->pali_isc_mode = ISC_STRICT;
      goto pali_isc_mode_done;
    }
  opt->pali_isc_mode = ISC_BASIC;
